...
func marshalMapPrefix(length int) []byte {
	...
	case length <= 23:
		switch length {
		case  0:
			result = append(result, initialbyte.MapLen0)
		case  1:
			result = append(result, initialbyte.MapLen1)
		case  2:
			result = append(result, initialbyte.MapLen2)
		case  3:
			result = append(result, initialbyte.MapLen3)
		case  4:
			result = append(result, initialbyte.MapLen4)
		case  5:
			result = append(result, initialbyte.MapLen5)
		case  6:
			result = append(result, initialbyte.MapLen6)
		case  7:
			result = append(result, initialbyte.MapLen7)
		case  8:
			result = append(result, initialbyte.MapLen8)
		case  9:
			result = append(result, initialbyte.MapLen9)
		case 10:
			result = append(result, initialbyte.MapLen10)
		case 11:
			result = append(result, initialbyte.MapLen11)
		case 12:
			result = append(result, initialbyte.MapLen12)
		case 13:
			result = append(result, initialbyte.MapLen13)
		case 14:
			result = append(result, initialbyte.MapLen14)
		case 15:
			result = append(result, initialbyte.MapLen15)
		case 16:
			result = append(result, initialbyte.MapLen16)
		case 17:
			result = append(result, initialbyte.MapLen17)
		case 18:
			result = append(result, initialbyte.MapLen18)
		case 19:
			result = append(result, initialbyte.MapLen19)
		case 20:
			result = append(result, initialbyte.MapLen20)
		case 21:
			result = append(result, initialbyte.MapLen21)
		case 22:
			result = append(result, initialbyte.MapLen22)
		case 23:
			result = append(result, initialbyte.MapLen23)
		}

	case length < 256: // 2^8
		result = append(result, initialbyte.MapLenUint8, byte(length))

	case length < 65536: // 2^16
		result = append(result, initialbyte.MapLenUint16,
			byte((length & 0xFF_00) >> 8),
			byte((length & 0x00_FF)     ),
		)

	case length < 4294967296: // 2^32
		result = append(result, initialbyte.MapLenUint32,
			byte((length & 0xFF_00_00_00) >> (8*3)),
			byte((length & 0x00_FF_00_00) >> (8*2)),
			byte((length & 0x00_00_FF_00) >> (8*1)),
			byte((length & 0x00_00_00_FF)         ),
	...
	default:
		var x uint64 = uint64(length)
		result = append(result, initialbyte.MapLenUint64,
			byte((x & 0xFF_00_00_00_00_00_00_00) >> (8*7)),
			byte((x & 0x00_FF_00_00_00_00_00_00) >> (8*6)),
			byte((x & 0x00_00_FF_00_00_00_00_00) >> (8*5)),
			byte((x & 0x00_00_00_FF_00_00_00_00) >> (8*4)),
...
func MarshalMap(value map[any]any) ([]byte, error) {

	length := len(value)

	var result []byte
	result = append(result, marshalMapPrefix(length)...)

	pairs := make([]encodedPair, 0, length)

	for k, v := range value {
		encodedKey, err := MarshalItem(k)
		if nil != err {
			return nil, fmt.Errorf("rfc8949: map key: %w", err)
		}

		encodedValue, err := MarshalItem(v)
		if nil != err {
			return nil, fmt.Errorf("rfc8949: map value: %w", err)
		}
	...
	// RFC 8949 Section 4.2.1: deterministic encoding requires keys sorted
	// by bytewise lexicographic order of their encoded forms.
	sort.Slice(pairs, func(i, j int) bool {
		return bytes.Compare(pairs[i].encodedKey, pairs[j].encodedKey) < 0
	})

	for _, pair := range pairs {
		result = append(result, pair.encodedKey...)
		result = append(result, pair.encodedValue...)
	}

	return result, nil
}

func MarshalMapStringKeys(value map[string]any) ([]byte, error) {
	...
	for k, v := range value {
		encodedKey, err := textstrings.Marshal(k)
		if nil != err {
			return nil, fmt.Errorf("rfc8949: map key: %w", err)
		}
		...
		encodedValue, err := MarshalItem(v)
		if nil != err {
			return nil, fmt.Errorf("rfc8949: map value for key %q: %w", k, err)
		}
